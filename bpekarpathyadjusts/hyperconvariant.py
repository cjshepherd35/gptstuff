#try to just chunk out  the self attention and the mlp and add it back to the input. 
import torch
...
class Block(nn.Module):
    def __init__(self, num_embed, n_head):
        ...
        self.sa = nn.ModuleList([MultiheadAttention(n_head, head_size) for _ in range(expans_rate)])
        self.fs = nn.ModuleList([FeedForward(n_embed) for _ in range(expans_rate)])
    ...
    def forward(self, x):
        print(x.shape)
        x_splits = torch.chunk(x, chunks=expans_rate, dim=2)
        output_list = []
        mlpout = []
       

       #this for loop does not work i think
        for input_data, layer in zip(x_splits, self.sa):
            print("input data ",input_data.shape)
            out = layer(input_data)
            output_list.append(out)
        # print(x.shape)
        outer = torch.cat(output_list, dim=-1)
        # print(outer.shape)
        x = x + self.ln1(outer)
        x_splits = torch.chunk(x, chunks=expans_rate, dim=-1)
        for input_data, layer in zip(x_splits, self.fs):
            outs = layer(input_data)
            mlpout.append(outs)
        x = x + self.ln2(torch.cat(mlpout, dim=-1))
        return x
...
class Transformer(nn.Module):
    ...
    def __init__(self):
        ...
        self.blocks = nn.Sequential(*[Block(n_embed*expans_rate, n_head) for _ in range(n_layer)])
