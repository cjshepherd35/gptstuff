#this can make the network shallower, it makes more parameters though. However using a sequential of Feedforwards without normalizing in between does not seem to help. 
import torch
...
class FeedForward(nn.Module):
    def __init__(self, n_embed):
        ...
        self.net = nn.Sequential(
            nn.Linear(n_embed, 6*n_embed),
            nn.ReLU(), 
            nn.Linear(6*n_embed, n_embed), 
            nn.Dropout(dropout)
        )
    def forward(self, x):
        return self.net(x)
...
class Block(nn.Module):
    def __init__(self, n_embed, n_head):
        ...
        # self.ffwds = nn.Sequential(*[FeedForward(n_embed) for _ in range(num_mlp_recurs)])   #this  one is no good. might have to see a way of looping
                                #while doing layer norms between each feedforward. 
