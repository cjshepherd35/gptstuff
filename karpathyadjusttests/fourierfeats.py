#trying to apply fourier features to increase dimensionality of the input data of  a transformer rather than use embeddings
#does not work
import torch
...
class BigramLanguageModel(nn.Module):
    ...
    def apply_fourier_features(self, x):
        FOURIER_DIM = n_embed //2
        FOURIER_SCALE = 10.
        INPUT_DIMS = FOURIER_DIM
        B = FOURIER_SCALE * torch.randn(size=(x.shape[2], FOURIER_DIM), requires_grad=False)
    ...
    def forward(self, idx, targets=None):
        ...
        idx = idx.unsqueeze(-1)
        token_embed = self.apply_fourier_features(idx).to(device)
