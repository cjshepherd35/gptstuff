#doesn't seem to help much at minimal training with small model. i cant say for bigger model and training
#making qkv nets have feedforward does not help either. 
import torch
...
class RePU(nn.Module):
    ...
    def __init__(self, initial_a=0.0, initial_b=1.0, initial_c=0.0, initial_d=0.0):
...
class Head(nn.Module):
    ...
    def __init__(self, head_size):
        super().__init__()
        self.key = FeedForward(n_embed, head_size)
        self.query = FeedForward(n_embed, head_size)
        self.value = FeedForward(n_embed, head_size)
...
class FeedForward(nn.Module):
    def __init__(self, n_embed, outputsize):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(n_embed, 4*n_embed),
            RePU(), 
            nn.Linear(4*n_embed, outputsize), 
            nn.Dropout(dropout)
        )
    def forward(self, x):
        return self.net(x)
