#smaller block of layers but you run through the recursion twice. is better than the standard MOR of double amt of layers
import torch
...
# --- MoR Transformer with Recursive Logic ---
...
class MoRTransformer(nn.Module):
    def __init__(self):
        ...
        # We instantiate only ONE block that will be reused
        self.blocks = nn.Sequential(*[Block(n_embed, n_head=n_head) for _ in range(n_layer)])
        self.blocks2 = nn.Sequential(*[Block(n_embed, n_head=n_head) for _ in range(n_layer)])
    ...
    def forward(self, idx, targets=None):
        ...
        for depth in range(max_recursion_depth):
            ...
            current_x = processed_x[halt_prob > probgetrid]
        ...
        for depth in range(max_recursion_depth):
            final_output = self.blocks2(final_output)
