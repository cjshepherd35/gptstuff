#changing head_dim to one for k,v, and q and see what happens. hopefully it will speed up training but likely will hurt performance
...
class MultiheadAttention(nn.Module):
    def __init__(self,num_heads, head_size):
        ...
        self.proj = nn.Linear(num_heads*head_size, n_embed)              #change here from n_embedxn_embed to num_headsxn_embed
        self.dropout = nn.Dropout(dropout)
    def forward(self, x):
...
class Block(nn.Module):
    def __init__(self, n_embed, n_head):
        super().__init__()
        head_size = 4                      #change here from head_size= n_embed//n_head to 1.
        self.sa = MultiheadAttention(n_head, head_size)
        self.ffwd = FeedForward(n_embed)
        self.ff2 = FeedForward(n_embed)
