#this is based off of deepmind paper reusing transformers in a large language model. it works!the loss goes down the same on smaller model
#to be more accurate, the loss is nearly the same when doing 2  layers recursed 4 times as when you  do 4 layers recursed 2 times. 
import torch
...
sample = textraw['train'].select(range(60_000))
rows = sample["text"]
