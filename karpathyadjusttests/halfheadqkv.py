#at 128 embedding dim it is about 20% less parameters and just as good. 
#seems like just reducing k and q to smaller works almost just as well.
import torch
...
class Head(nn.Module):
    ...
    def __init__(self, head_size):
        super().__init__()
        self.key = nn.Linear(n_embed, head_size//2, bias=False)
        self.query = nn.Linear(n_embed, head_size//2, bias=False)
        self.value = nn.Linear(n_embed, head_size//2, bias=False)
        self.register_buffer('tril', torch.tril(torch.ones(block_size, block_size)))
        self.dropout = nn.Dropout(dropout)

    def forward(self, x):
        b,t,c = x.shape
        k = torch.cat((self.key(x), self.key(x)), dim=-1)
        q = torch.cat((self.query(x), self.query(x)), dim=-1)
        # k = self.key(x)
        # q = self.query(x)
        ...
        v = torch.cat((self.value(x), self.value(x)), dim=-1)
        # v = self.value(x)
        out = wei @ v
        return out
